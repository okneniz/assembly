...
func TestWriteFlags(t *testing.T) {
	...
	if got := binary.LittleEndian.Uint32(out[48:52]); got != 0x43 {
		t.Fatalf("e_flags = %#x, want 0x43", got)
	}
}
...
func TestWriteMemszTail(t *testing.T) {
	...
	out, err := elf.Write(elf.EM_RISCV, 0, base, base, []elf.Blob{blob})
	...
	require.Equal(t, int(0x10000+4+56), len(out), "the tail occupies no file space")
}

func TestWriteMemszTailNotLast(t *testing.T) {
	b1 := elf.NewNobitsBlob(0x1000, []byte{1}, 0x10)
	b2 := elf.NewBlob(0x2000, []byte{2})
	_, err := elf.Write(elf.EM_RISCV, 0, 0x1000, 0x1000, []elf.Blob{b1, b2})
