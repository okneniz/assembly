// Command assembly assembles a source file (a GNU-as compatible subset
...
// Usage:
//
//	assembly -arch riscv64 -base 0x1000 source.s -o prog.bin --sym symbols.txt
//	assembly -arch arm64 --hex source.s
//	assembly -arch riscv64 -base 0x1000 < roundtrip.s --hex
package main
...
func main() {
	...
	formatFlag := flag.String(
		"format",
		"raw",
		"output format: raw (default), elf — a minimal static ELF64 (runs natively on Linux arm64/riscv64/loong64 or under qemu), macho — an arm64 macOS executable (runs as-is on Apple Silicon, ad-hoc signed by the writer)",
	)
	flag.Usage = func() {
		fmt.Fprintln(
			os.Stderr,
			"usage: assembly -arch ARCH [-base ADDR] [-o FILE] [--sym FILE] [--hex] [SOURCE]",
		)
	...
	if *disasmFlag {
		...
		if err := runDisasm(src, arch, base); err != nil {
	...
	if *outPath != "" && len(errs) == 0 {
		...
		switch *formatFlag {
		case "elf":
			machine, flags, merr := elfMachine(arch)
			if merr != nil {
...
// assembly): disasm assembles the lines from the ObjDump text of the
