// Package pseudo provides RISC-V pseudo-instructions (GNU as + the
// ObjDump text's own aliases): a layer ABOVE asm/riscv (the grammar)
// and arch/riscv (the real instructions). A pseudo-instruction is
...
//   - multi-word (la/call/tail) - their own evaluated forms with a
//     fixed length: RVC pcrel pairs must not be split, for
//     deterministic assembler passes;
//   - single-word (nop/mv/not/...) - expansion into base forms and
//     the syntax layer's ResolveForm; compressed by the general RVC
//     rules.
package pseudo
...
func newPInstr(mnem string, ops []riscv.Op, be *riscv.Backend) pInstr {
	return pInstr{
		mnem: mnem,
		ops:  ops,
		be:   be,
...
func (p pInstr) resolve(ctx asm.Ctx) (asm.Resolved, error) {
	...
	// single-word ones expand into base forms + syntax layer forms
	forms, err := expandPseudo(p)
	if err != nil {
		return nil, err
	}

	forms = p.preferAddFormMv(forms, ctx)
