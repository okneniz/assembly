...
func decodeFsubD(w uint32, addr uint64) Instr {
	return FsubD{
		base: newBase(addr, w),
...
func (i FsubD) ObjDump(_ disasm.ViewCtx) string {
	return fmt.Sprintf("fsub.d %s, %s, %s", i.rd, i.rs1, i.rs2)
}

func (i FsubD) Encode(w io.Writer, pc uint64, o EncOpts) (int64, error) {
	rm := i.rm.val
	...
	return writeWord(w, riscvEncodings["fsub_d"][0]|uint32(rm)<<12|
