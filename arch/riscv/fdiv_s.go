package riscv

import (
	"fmt"
	"io"

	"github.com/okneniz/assembly/disasm"
)

// FdivS - fdiv.s fd, fs1, fs2.
type FdivS struct {
	base

	rd, rs1, rs2 string
	rm           imm // rounding mode (not shown in text)
}

// FdivS - fdiv.s fd, fs1, fs2; the registers are FP registers taken by
// number (Reg 0..31 is printed ft0/fa0/...); rm is the rounding mode 0..7
// (0 RNE, 1 RTZ, 2 RDN, 3 RUP, 4 RMM, 7 DYN).
func (Builder) FdivS(rd, rs1, rs2 Reg, rm uint8) Instr {
	return FdivS{
		rd:  fpName(rd),
		rs1: fpName(rs1),
		rs2: fpName(rs2),
		rm:  immNum(int64(rm)),
	}
}

func decodeFdivS(w uint32, addr uint64) Instr {
	return FdivS{
		base: newBase(addr, w),
		rd:   rvFRegNames[w>>7&0x1f],
		rs1:  rvFRegNames[w>>15&0x1f],
		rs2:  rvFRegNames[w>>20&0x1f],
		rm:   immNum(int64(w >> 12 & 0x7)),
	}
}

func (i FdivS) ObjDump(_ disasm.ViewCtx) string {
	return fmt.Sprintf("fdiv.s %s, %s, %s", i.rd, i.rs1, i.rs2)
}

func (i FdivS) Encode(w io.Writer, pc uint64, o EncOpts) (int64, error) {
	rm := i.rm.val

	if rm < 0 || rm > 7 {
		return 0, fmt.Errorf("rounding mode %d out of range", rm)
	}

	return writeWord(w, riscvEncodings["fdiv_s"][0]|uint32(rm)<<12|
		fregBits(i.rd)<<7|fregBits(i.rs1)<<15|fregBits(i.rs2)<<20)
}

func (i FdivS) MarshalJSON() ([]byte, error) {
	return i.marshalDTO(
		"fdiv.s",
		i.ObjDump(disasm.DefaultViewCtx()),
		"RV32F",
		map[string]any{"rd": i.rd, "rs1": i.rs1, "rs2": i.rs2},
	)
}

func newFdivS(ops []Op) (Instr, error) {
	regs, err := wantFP(ops, "fdiv.s", 3)
	if err != nil {
		return nil, err
	}

	return FdivS{
		rd:  regs[0],
		rs1: regs[1],
		rs2: regs[2],
		rm:  wantRM(ops, 3),
	}, nil
}
