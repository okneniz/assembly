...
func (i AmomaxD) Encode(w io.Writer, pc uint64, o EncOpts) (int64, error) {
	return writeWord(w, riscvEncodings["amomax_d"][0]|
		regBits(i.rd)<<7|regBits(i.rs1)<<15|regBits(i.rs2)<<20)
}

func (i AmomaxD) MarshalJSON() ([]byte, error) {
	return i.marshalDTO(
		"amomax.d",
...
func newAmomaxD(ops []Op) (Instr, error) {
	if len(ops) != 3 {
		return nil, errors.New("amomax.d: want rd, rs2, (rs1)")
	}

	rd, err := wantReg(ops[0], false)
	if err != nil {
		return nil, fmt.Errorf("amomax.d: %w", err)
	}
	...
	base, _, err := wantMem(ops[2], "amomax.d")
	if err != nil {
		return nil, err
	}
