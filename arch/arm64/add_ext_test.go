...
func TestAddExtBuild(t *testing.T) {
	cases := []struct {
		...
		{
			"add x1,x2,x3,uxtb#0",
			buildAddExt(t, xreg(t, 1), xreg(t, 2), xreg(t, 3), "uxtb", 0),
			0x8b230041,
		},
		{
			"add x0,x1,x2,uxtx#7",
			buildAddExt(t, xreg(t, 0), xreg(t, 1), xreg(t, 2), "uxtx", 7),
			0x8b227c20,
		},
		{
			"add w1,w2,w3,uxtw#2",
			buildAddExt(t, wreg(t, 1), wreg(t, 2), wreg(t, 3), "uxtw", 2),
			0x0b234841,
		},
		{
			"add sp,sp,x3,sxtx#3",
			buildAddExt(t, SP, SP, xreg(t, 3), "sxtx", 3),
			0x8b23efff,
		},
	}
	for _, c := range cases {
	...
	in := buildAddExt(t, xreg(t, 1), xreg(t, 2), xreg(t, 3), "uxtb", 0)
	_, ok := in.(AddExt)
...
// buildAddExt — an instruction constructor wrapper for table literals:
// valid operands, an error is impossible by construction.
func buildAddExt(t *testing.T, rd, rn, rm Reg, ext string, imm3 uint32) Instr {
	t.Helper()
