...
func (Builder) Fp2(op string, rd, rn string, enc uint32, rdK, rnK FpKind) Instr {
	return Fp2{op: op, rd: rd, rn: rn, enc: enc, rdK: rdK, rnK: rnK}
}

// Fcmp — fcmp/fcmpe: withRM selects the register form (vs #0.0).
func (Builder) Fcmp(fn, fm string, withRM bool, enc0, encR uint32, k FpKind) Instr {
	return Fcmp{rn: fn, rm: fm, withRM: withRM, enc0: enc0, encR: encR, k: k}
}

// FmovImm — fmov rd, #literal: text is the literal's source spelling
// (the struct prints it back verbatim).
func (Builder) FmovImm(rd string, val float64, text string, isS bool, enc uint32, rdK FpKind) Instr {
	return FmovImm{rd: rd, val: val, text: text, isS: isS, enc: enc, rdK: rdK}
}
