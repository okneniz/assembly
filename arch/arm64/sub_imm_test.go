package arm64

import (
	"testing"

	"github.com/stretchr/testify/require"
)

func TestSubImmBuild(t *testing.T) {
	got := buildWord(
		t,
		buildSubImm(t, xreg(t, 0), xreg(t, 1), imm12(t, 0x42), NoSh12),
	)
	require.Equal(t, uint32(0xd1010820), got, "sub")
	got = buildWord(t, buildSubImm(t, wreg(t, 0), WSP, imm12(t, 1), NoSh12))
	require.Equal(t, uint32(0x510007e0), got, "sub w0,wsp")
	in := buildSubImm(t, xreg(t, 0), xreg(t, 1), imm12(t, 1), NoSh12)
	_, ok := in.(SubImm)
	require.True(t, ok, "type = %T, want SubImm", in)
	_, err := New().SubImm(XZR, xreg(t, 1), imm12(t, 1), NoSh12)
	assertErr(t, "sub imm zr", err)
	_, err = New().SubImm(xreg(t, 0), XZR, imm12(t, 1), NoSh12)
	assertErr(t, "sub imm rn zr", err)
	_, err = New().SubImm(xreg(t, 0), wreg(t, 1), imm12(t, 1), NoSh12)
	assertErr(t, "sub imm x+w", err)
}
