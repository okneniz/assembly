...
func TestEonShiftBuild(t *testing.T) {
	cases := []struct {
		...
		{
			"eon x1,x2,x3,ror#5",
			buildEonShift(t, xreg(t, 1), xreg(t, 2), xreg(t, 3), imm6(t, 5), ROR),
			0xcae31441,
		},
		{
			"eon w1,w2,w3",
			buildEonShift(t, wreg(t, 1), wreg(t, 2), wreg(t, 3), imm6(t, 0), LSL),
			0x4a230041,
		},
	}
	for _, c := range cases {
	...
	in := buildEonShift(t, xreg(t, 1), xreg(t, 2), xreg(t, 3), imm6(t, 1), LSL)
	_, ok := in.(EonShift)
...
// buildEonShift — an instruction constructor wrapper for table literals:
// valid operands, an error is impossible by construction.
func buildEonShift(t *testing.T, rd, rn, rm Reg, imm Imm6, sh Shift) Instr {
	t.Helper()
