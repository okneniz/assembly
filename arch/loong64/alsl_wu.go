...
// NewAlslWu - alsl.wu rd, rj, rk, shift.
func NewAlslWu(rd, rj, rk Reg, shift Shift3) Instr {
	return AlslWu{
		rd:    rd.Num(),
		rj:    rj.Num(),
		rk:    rk.Num(),
...
func decodeAlslWu(w uint32, addr uint64) Instr {
	return AlslWu{
		base:  newBase(addr, w),
		rd:    uint8(w & 0x1f),
		rj:    uint8(w >> 5 & 0x1f),
...
func (i AlslWu) Encode(w io.Writer, _ uint64) (int64, error) {
...
func (i AlslWu) MarshalJSON() ([]byte, error) {
	return i.marshalDTO(
		"alsl.wu",
