...
// AmcasDbB - amcas_db.b rd, rk, rj (DJK, @orig_fmt=DKJ - the second assembly
...
// NewAmcasDbB - amcas_db.b rd, rk, rj (the operand order the assembler writes).
func NewAmcasDbB(rd, rk, rj Reg) Instr {
	return AmcasDbB{
		rd: rd.Num(),
		rk: rk.Num(),
		rj: rj.Num(),
