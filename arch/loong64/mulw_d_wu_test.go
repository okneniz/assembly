...
func TestMulwDWuCtor(t *testing.T) {
	// llvm-mc-verified: mulw.d.wu $t0, $t1, $t2.
	require.Equal(
		t,
		uint32(0x001fb9ac),
		ctorWord(t, New().MulwDWu(lreg(t, 12), lreg(t, 13), lreg(t, 14))),
	)

	in := New().MulwDWu(lreg(t, 1), lreg(t, 2), lreg(t, 3))
	_, ok := in.(MulwDWu)
