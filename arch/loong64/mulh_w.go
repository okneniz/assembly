...
// NewMulhW - mulh.w rd, rj, rk.
func NewMulhW(rd, rj, rk Reg) Instr {
	return MulhW{
		rd: rd.Num(),
		rj: rj.Num(),
		rk: rk.Num(),
...
func decodeMulhW(w uint32, addr uint64) Instr {
	return MulhW{
		base: newBase(addr, w),
		rd:   uint8(w & 0x1f),
		rj:   uint8(w >> 5 & 0x1f),
...
func (i MulhW) Encode(w io.Writer, _ uint64) (int64, error) {
...
func (i MulhW) MarshalJSON() ([]byte, error) {
	return i.marshalDTO(
		"mulh.w",
