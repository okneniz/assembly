...
// AmminDbW - ammin_db.w rd, rk, rj (DJK, @orig_fmt=DKJ - the second assembly
...
// NewAmminDbW - ammin_db.w rd, rk, rj (the operand order the assembler writes).
func NewAmminDbW(rd, rk, rj Reg) Instr {
	return AmminDbW{
		rd: rd.Num(),
		rk: rk.Num(),
		rj: rj.Num(),
...
func decodeAmminDbW(w uint32, addr uint64) Instr {
	return AmminDbW{
		base: newBase(addr, w),
		rd:   uint8(w & 0x1f),
		rk:   uint8(w >> 10 & 0x1f),
...
func (i AmminDbW) Encode(w io.Writer, _ uint64) (int64, error) {
...
func (i AmminDbW) MarshalJSON() ([]byte, error) {
	return i.marshalDTO(
		"ammin_db.w",
